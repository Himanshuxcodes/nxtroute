...
export default function RouteGenerator({
  ...
  const handleCreate = async () => {
    if (!topic.trim() || loading || isCreating.current) return;
      ...
      <div className="bg-white w-full max-w-lg rounded-[2.5rem] p-10 shadow-2xl border border-slate-100 animate-in fade-in zoom-in duration-300">
        ...
        <div className="flex gap-4">
          <button
            ...
            onClick={onClose}
            disabled={loading}
            className="flex-1 py-5 bg-slate-100 text-slate-500 rounded-2xl font-black uppercase tracking-widest text-[10px] disabled:opacity-50"
          >
            Cancel
          </button>
          ...
          <button
            ...
            className="flex-[2] py-5 bg-indigo-600 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 disabled:opacity-50"
          >
