...
router.post('/create', async (req, res) => {
  try {
    ...
    if (requestId) {
      const existing = await Interview.findOne({ requestId });
      if (existing) {
    ...
    if (!questions || questions.length === 0) {
      return res.status(500).json({
        message: 'AI failed to generate questions. Try again.'
      });
    }
...
router.post('/submit-assessment', async (req, res) => {
  try {
    const { interviewId, ...rest } = req.body;

    const interview = await Interview.findById(interviewId);
    const recruiterId = interview ? interview.recruiterId : null;
